...
const startServer = async () => {
  try {
    ...
    app.use(
      helmet.contentSecurityPolicy({
        directives: {
          ...
          ], // Разрешено только загружать ресурсы с текущего домена
          scriptSrc: [
            "'self'",
            "'unsafe-inline'",
            "https://api-maps.yandex.ru",
            "https://www.gstatic.com/charts/loader.js",
            "https://yastatic.net/s3/",
    ...
    app.use(
      express.static(
        path.join("C:/Users/Администратор/RIAS/ris-app-client/build")
      )
    );
    app.get("*", (req, res) => {
      res.sendFile(
        path.join("C:/Users/Администратор/RIAS/ris-app-client/build/index.html")
      );
    });
